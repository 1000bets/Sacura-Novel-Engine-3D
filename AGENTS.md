...
- **EditorCore** (CMake `EditorCore`) depends on Engine + ufbx — import tools, `SceneDocument`, `EditorCommandStack` / scene edit commands, abstract reflected `editor.EditorAction`. Its operations require no QApplication; Qt linkage is inherited from Engine. It is built independently of SAKURA_BUILD_EDITOR.
- **Editor** (logical QtEditorFramework, CMake `Editor`) depends on EditorCore + Qt6 Widgets + ImGuizmo — shell UI, adapters, native viewport host, edit-only transform manipulator, concrete `editor.*` create/add actions (`BuiltinEditorActions`, `ForceTouchEditorRegistrars` before Engine init). Entry: `SakuraEditor` (`Root/Editor/apps/SakuraEditorMain.cpp`).
...
- EditorCore commands (`EditorCommandStack` + `Make*Command`): rename, transform, create/delete (subtree JSON undo), reparent (cycle reject), add/remove component, set/reset property. All Hierarchy/Inspector mutations go through the stack; stack marks the document dirty. Hierarchy drag-and-drop does not move Qt items in place: drop onto an item parents the dragged object to it, drop between items uses that item's parent (or scene root), cycles are rejected, and the tree is rebuilt from the scene. The drop action is forced to Ignore/Copy so Qt does not delete the dragged row after the scene reparent. Selection uses `ObjectHandle`, never Qt ownership of `GameObject`.
...
- Only `Rendering/RHI` may talk to Diligent. Editor ImGuizmo uses DiligentTools' bundled ImGui on the Game thread; GPU overlay submit goes through RHI `ImGuiDiligentRenderer` (never from Editor/Qt).
...
- `ContentBrowserWidget` follows an Unreal-like sources/assets layout: Engine and Game folder trees, compact registered-type filter buttons, search and draggable asset tiles. `All` shows the direct contents of the selected folder; an active type filter shows every matching final asset recursively under that folder. It displays only final usable assets: storage containers such as `Model` are hidden when they own subassets, and every non-empty GLB mesh is shown as its own `StaticMesh` tile. Final asset names are case-insensitively unique across the Game project; import derives stable names from the destination file and adds numeric suffixes on conflicts. Game Content accepts imports through the Import button or external file drop. Both open `AssetImportDialog` for destination names/folder and format-specific options before invoking `AssetImporter`; Engine Content is read-only. Game folders support create / F2 rename / Delete (nested folders only; `/Game` and Engine stay protected), including empty folders discovered on disk. Supported UI imports are self-contained GLB, static FBX (converted to GLB, optional missing-normal generation), PNG and JPEG. Model import writes one `StaticMesh` subasset record per non-empty GLB mesh (`selector.kind="mesh"`, source mesh index). Game assets support F2/context-menu rename, confirmed Delete, and drag-and-drop moves onto Game folders; siblings sharing one backing file move together, deleting the last leaf removes the data/metadata pair, GUIDs stay stable across rename/move, and deletion unregisters CPU slots and retires cached GPU resources. It exports `application/x-sakura-asset`; dropping a StaticMesh into the Scene viewport creates a `GameObject` with `MeshRendererComponent`, while dropping a Material assigns it to the selected mesh object.
- Clipboard shortcuts use the system Qt clipboard. Text inputs, spin boxes and editable combos retain native Qt Ctrl+C/Ctrl+V behavior. Hierarchy and the Edit-mode viewport copy the selected object subtree as `application/x-sakura-scene-subtree`; paste duplicates it as a sibling through `MakePasteSubtreeCommand`, remaps every object/component persistent ID and remains undoable. Content Browser copy stores `application/x-sakura-asset`; paste duplicates the backing file and metadata into the selected Game folder with fresh asset/subasset GUIDs and unique final names. Engine Content remains read-only as a paste destination.
- Scene gizmo uses ImGuizmo with exclusive Translate / Rotate / Scale modes (`W` / `E` / `R` physical scan codes and matching viewport-header buttons next to Camera Speed). It is available only while the single viewport is in Edit mode. During a drag the selected object receives preview transforms directly; release restores the old value and commits one `MakeSetTransformCommand`, preserving a single undo step. Edit-mode object picking constructs a camera ray on the CPU and selects the nearest visible `MeshRendererComponent` by its transformed world AABB; it does not require PhysX colliders and does not run during Play or gizmo manipulation. Inspector transforms expose position, Euler rotation in degrees and scale. The component selector is a filter: `All` is the default and displays every component in its own settings card; selecting a component type only hides non-matching cards. Edit-mode viewport camera is Unreal-like: RMB look + WASD/QE fly (Shift faster, Ctrl slower, wheel adjusts speed), MMB pan, wheel dolly, Alt+LMB orbit, Alt+RMB dolly; Play mode switches to the game camera and disables editor navigation. Create / Hierarchy menus are built from reflected `editor.EditorAction` types (`EditorAction::CollectPublishedActions`); builtins live under `Root/Editor/inc/Actions/` with categories `Objects` / `Components`. Project-side native actions can subclass `EditorAction`, register via `ENGINE_CLASS`, and call `ForceTouchEditorRegistrars` (or link their TU) before `InitializeNative`.

## Coding conventions (engine)

- Always bind keyboard controls to physical key positions using native scan codes, independent of the active keyboard layout. This includes editor navigation and gameplay controls. Do not fall back to character-based key values for these controls. Keep standard text input and platform UI shortcuts under Qt's normal handling.
- Editor gizmo overlay is a non-activating, input-transparent tool window used only for ImGuizmo input/state. The native viewport owns mouse/keyboard input and forwards mouse events in viewport coordinates to the gizmo. Pass SimpleMath matrices directly to ImGuizmo without transposing; both store translation in the last four-float row. ImGuizmo draw data is snapshotted into `ImGuiOverlaySnapshot` on the Game thread (`Engine::SetRenderSurfaceImGuiOverlay`), carried on `RenderViewFrame`, and drawn by `ImGuiDiligentRenderer` on the Render Thread after FXAA onto the swapchain. There is no QPainter ImGui drawer.
