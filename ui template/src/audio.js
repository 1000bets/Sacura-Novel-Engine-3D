...
export class SoundDesk {
 ...
 applyVolumes(notify=true){const gain=this.duckLevel();for(const t of this.tracks.values()){const volume=Math.max(0,Math.min(1,t.volume*t.envelope*(t.kind==='music'?gain:1)));if(t.audio.volume!==volume)t.audio.volume=volume;}if(notify)this.emit();}
 ...
 stopAll(){for(const key of this.tracks.keys())this.stop(key);this.ducks.clear();clearInterval(this.duckTimer);this.duckTimer=null;this.duckTransition=null;this.duckGain=this.duckTarget=1;this.refreshVolumes();}
