...
export function upgradeProject(source){
 ...
 for(const asset of AUDIO_ASSETS.filter(a=>a.kind==='voice')){const e=ev(asset.id,`${asset.speaker} · озвучка`,'sound',asset.id==='voice-bob'?'bob':asset.id==='voice-alice'?'alice':'world',asset.caption);Object.assign(e.groups[0].actions[0],{assetId:asset.id,volume:1,duck:true,duration:3});}
