...
const soundTrack=(audio,event)=>[...audio.tracks.values()].find(t=>t.key.endsWith(':'+event.groups[0].actions[0].id));
...
test('runtime really waits BEFORE, starts parallel audio together, then executes AFTER',async()=>{
 const makeVoice=name=>{const e=newEvent('sound','world',name,name);e.groups[0].actions[0].assetId='voice-narrator';return e;};const pre=makeVoice('pre'),left=makeVoice('left'),right=makeVoice('right'),after=newEvent('variable','trust','+1');const bs=[binding('pre',pre.id,'BEFORE'),binding('left',left.id),binding('right',right.id),binding('after',after.id,'AFTER')];const b=line('a',bs,null,{batches:{BEFORE:[{id:'before',mode:'SEQUENTIAL',bindingIds:['pre']}],ON_START:[{id:'parallel',mode:'PARALLEL',bindingIds:['left','right']}],AFTER:[{id:'after',mode:'SEQUENTIAL',bindingIds:['after']}]}});const p=sceneProject([pre,left,right,after],[b]),audio=new AudioMock(),runtime=new PreviewRuntime(audio);runtime.delay=async(_,token)=>runtime.assert(token);const start=runtime.start(p,'a');await tick();assert.equal(runtime.snapshot.textVisible,false);assert.equal(runtime.snapshot.variables.trust,0);soundTrack(audio,pre).finish();await tick();assert.equal(runtime.snapshot.textVisible,true);assert.ok(soundTrack(audio,left));assert.ok(soundTrack(audio,right));assert.equal(runtime.snapshot.ready,false);soundTrack(audio,left).finish();await tick();assert.equal(runtime.snapshot.ready,false);soundTrack(audio,right).finish();await start;assert.equal(runtime.snapshot.ready,true);await runtime.advance();assert.equal(runtime.snapshot.variables.trust,1);assert.equal(runtime.snapshot.phase,'FINISHED');runtime.stop();
});
